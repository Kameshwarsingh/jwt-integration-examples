...
We will use authorization-code grant-type. Sample code is to illustrate JWT flow, this code must be used only as an example. Although for illustration this example uses HTTP (and localhost as domain), in real world you must use HTTPS, else token/s will be easily stolen while in transit.
